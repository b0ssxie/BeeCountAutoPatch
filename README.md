...
> 只有保存「分类名单」需要 root——hook 侧要读的那个文件在自动记账的私有目录里。日志和名单读取都不再需要 root。
...
模块日志直接进 **LSPosed 自带的日志页**（用的是新版 API 的 `log()`），不再写文件、也不需要 root：

1. 打开 LSPosed →「日志」；
2. 搜 `[BeeCountAutoPatch]`，能看到：hook 是否生效、适配器注入、解析到的蜜蜂记账包名、每次写入的类型/金额/分类/生成的 URI、Intent 是否可被处理。

常见现象对照：

| 现象 | 原因 |
| --- | --- |
| LSPosed 日志里搜不到 `[BeeCountAutoPatch]` | 模块没被加载：LSPosed 里没启用本模块，或框架不支持新版 API（`minApiVersion=101`） |
| 记账应用列表里看不到「蜜蜂记账」 | 模块未生效，或自动记账不是 4.0.x |
...
> v1.2.x 曾把日志写进自动记账的私有目录、再由本 App 读（依赖 root，经常读不到，所以表现像「日志没用」）。v1.4.0 起改成直接输出到 LSPosed 日志，本 App 界面只保留分类名单维护。
