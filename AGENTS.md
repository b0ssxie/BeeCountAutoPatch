...
Release builds are minified. `BeeCountHook` is loaded by name from `assets/xposed_init`, so never drop the keep rules in `app/proguard-rules.pro`.
...
JUnit 4 lives in `app/src/test/...`. Test names use `subject_expectedBehavior` (e.g. `buildUri_transferHasNoCategoryAndUsesToAccount`). Prefer pure-JVM logic over mocking Android. Run `gradle testDebugUnitTest` before a PR; no coverage threshold is enforced.
